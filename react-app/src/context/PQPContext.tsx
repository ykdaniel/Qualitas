...
export const PQPProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  }, [isAuthenticated, fetchPQPs]);

  const addPQP = useCallback(async (pqp: Omit<PQPItem, 'id'>): Promise<PQPItem> => {
    try {
  ...
  const updatePQP = useCallback(async (id: string, updates: Partial<PQPItem>) => {
    try {
  ...
  const deletePQP = useCallback(async (id: string) => {
    try {
  ...
  const getPQPList = useCallback(() => pqpList, [pqpList]);
  const getPQPByVendor = useCallback((vendor: string) => pqpList.filter(pqp => pqp.vendor === vendor), [pqpList]);

  const value = useMemo(
    () => ({ pqpList, loading, error, refetch: fetchPQPs, addPQP, updatePQP, deletePQP, getPQPList, getPQPByVendor }),
    [pqpList, loading, error, fetchPQPs, addPQP, updatePQP, deletePQP, getPQPList, getPQPByVendor]
  );
