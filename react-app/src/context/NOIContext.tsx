...
export const NOIProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  }, [isAuthenticated, fetchNOIs]);

  const addNOI = useCallback(async (noi: Omit<NOIItem, 'id'>, id?: string): Promise<NOIItem> => {
    try {
  ...
  const addBulkNOI = useCallback(async (nois: Omit<NOIItem, 'id'>[]): Promise<NOIItem[]> => {
    try {
  ...
  const updateNOI = useCallback(async (id: string, updates: Partial<NOIItem>) => {
    try {
  ...
  const deleteNOI = useCallback(async (id: string) => {
    try {
  ...
  const getNOIList = useCallback(() => noiList, [noiList]);
