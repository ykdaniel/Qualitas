...
export const ITRProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const getITRList = useCallback(() => itrList, [itrList]);

  const getITRByNOI = useCallback((noiNumber: string) => {
    return itrList.filter(itr => itr.noiNumber === noiNumber);
  }, [itrList]);

  const getITRByNCR = useCallback((ncrNumber: string) => {
  ...
  const value = useMemo(
    ...
    [itrList, loading, error, fetchITRs, addITR, updateITR, deleteITR, getITRList, getITRByNOI, getITRByNCR]
  );
