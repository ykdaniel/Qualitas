...
export const OBSProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const getOBSList = useCallback(() => obsList, [obsList]);
  const getOBSByVendor = useCallback((vendor: string) => obsList.filter(obs => obs.vendor === vendor), [obsList]);

  const value = useMemo(
    () => ({ obsList, loading, error, refetch: fetchOBSs, addOBS, updateOBS, deleteOBS, getOBSList, getOBSByVendor }),
    [obsList, loading, error, fetchOBSs, addOBS, updateOBS, deleteOBS, getOBSList, getOBSByVendor]
  );
